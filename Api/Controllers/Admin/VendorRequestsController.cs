...
namespace Api.Controllers.Admin
{
    ...
    public class VendorRequestsController(ISender mediator) : ControllerBase
    {

        // --- VENDOR REQUESTS (ONBOARDING) ---
        [HttpGet("requests")]
        public async Task<ActionResult<List<VendorRequestDetailsDto>>> GetVendorRequests()
            => await Handle(new GetVendorRequestsQuery());

        [HttpGet("requests/{id}")]
        public async Task<ActionResult<VendorRequestDetailsDto>> GetVendorRequest(Guid id)
        ...
        [HttpGet]
        public async Task<ActionResult<List<VendorSummaryDto>>> GetAllVendors()
